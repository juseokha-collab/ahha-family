  ...
  const row = ev => {
    ...
    <div class="list-item" style="align-items:center;">
      ...
      <div style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;${completed?'text-decoration:line-through;opacity:0.6;':''}">
        ${(()=>{ const catName=eventCategoryOf(ev); if(!catName) return ''; const icon=EVENT_CATEGORY_ICONS[catName]; if(icon) return `<span style="position:relative;display:inline-block;width:20px;height:1em;vertical-align:middle;margin-right:4px;"><img src="${icon}" alt="${escapeHtml(catName)}" title="${escapeHtml(catName)}" style="position:absolute;left:0;top:50%;transform:translateY(-50%);width:32px;height:32px;border-radius:8px;object-fit:cover;"></span>`; const cat=eventCategories().find(c=>c.name===catName); const color=cat?cat.color:'rgba(255,105,150,0.14)'; return `<span class="pill" style="background:${color};margin-right:4px;">${escapeHtml(catName)}</span>`; })()}<span style="font-size:13px;font-weight:400;">${escapeHtml(ev.name)}</span>
